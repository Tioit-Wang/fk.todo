...
pub fn build_prompt(settings: &Settings, input: &AiPlanRequest, now: Timestamp) -> (String, String) {
    let system = [
    ...
    let custom = settings.ai_prompt.trim().to_string();
...
pub async fn plan_with_deepseek(settings: &Settings, input: &AiPlanRequest) -> Result<AiPlan, String> {
...
mod tests {
    ...
    fn parse_plan_accepts_fenced_json() {
        let plan = parse_plan_from_text("```json\n{\"notes\":\"n\",\"steps\":[\"a\"]}\n```").unwrap();
        assert_eq!(plan.notes, "n");
    ...
    fn parse_plan_dedupes_and_trims_steps() {
        let plan =
            parse_plan_from_text(r#"{"notes":" n ","steps":[" a ","a",""," b "]}"#).unwrap();
        assert_eq!(plan.notes, "n");
        assert_eq!(plan.steps, vec!["a".to_string(), "b".to_string()]);
    }
}
