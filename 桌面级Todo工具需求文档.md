# 必做清单（MustDo）需求文档（MVP）

## 1. 文档概述

### 1.1 文档目的

本文档明确必做清单（MustDo）的功能需求、交互逻辑、数据存储规则及设计规范，为开发、测试及设计工作提供统一依据，确保产品符合“快速记录、精准提醒、科学规划”的核心诉求。

### 1.2 产品定位

必做清单（MustDo）是一款轻量、高效、专注桌面场景的任务管理工具。通过快捷键唤醒、强制提醒、四象限分类、循环任务与筛选排序等能力，帮助用户梳理每日任务、把控优先级，提升时间管理效率。
...
1. 唤醒方式：支持自定义快捷键（默认推荐 `Ctrl+Shift+T`，可在设置中修改）。按下快捷键后，在桌面居中偏下位置弹出快捷窗口（内容区固定 500×650）。
...
5. 窗口状态记忆：
   - 快捷窗口位置：拖动窗口后，若 2 秒内无进一步拖动，自动保存当前位置，下次唤醒恢复（大小固定 500×650）。
...
1. 快捷窗口：内容区固定 500×650，不支持拖拽边缘调整大小；拖拽标题栏移动位置，下次唤醒保留。
